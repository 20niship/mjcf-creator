...
bool Mujoco::add_urdf(const std::string& urdf_path, const std::string& name_prefix, bool copy_meshes, const std::unordered_map<std::string, std::shared_ptr<BaseActuator>>& actuator_metadata) {
  if(!std::filesystem::exists(urdf_path)) {
    printf("URDF file does not exist: %s\n", urdf_path.c_str());
    return false;
  }

  return UrdfConverter::parse_urdf_to_mjcf(this, urdf_path, actuator_metadata, copy_meshes);
}
