...
std::tuple<std::shared_ptr<mjcf::Body>, std::shared_ptr<mjcf::Joint>> Mujoco::add_urdf(const std::string& urdf_path, [[maybe_unused]] const std::string& name_prefix, bool copy_meshes, const std::vector<std::shared_ptr<BaseActuator>>& actuator_metadata, const Arr3& pos, bool use_collision_tag_only) {
  if(!std::filesystem::exists(urdf_path)) {
    printf("URDF file does not exist: %s\n", urdf_path.c_str());
    return {nullptr, nullptr};
  }

  return UrdfConverter::parse_urdf_to_mjcf(this, urdf_path, pos, actuator_metadata, copy_meshes, "", use_collision_tag_only);
}
