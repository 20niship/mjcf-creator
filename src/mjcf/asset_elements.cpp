...
void Texture::set_xml_attrib() const {
  ...
  if(builtin != TextureBuiltin::None) this->set_attribute("builtin", to_string(builtin));
  if(type != TextureType::TwoD) this->set_attribute("type", to_string(type));
  if(!file.empty()) this->set_attribute("file", file);
  if(!name.empty()) this->set_attribute("name", name);
  if(width != 0) this->set_attribute("width", width);
  if(height != 0) this->set_attribute("height", height);

  if(rgb1 != std::array<double, 3>{0.0, 0.0, 0.0}) this->set_attribute("rgb1", std::vector<double>(rgb1.begin(), rgb1.end()));
  if(rgb2 != std::array<double, 3>{0.0, 0.0, 0.0}) this->set_attribute("rgb2", std::vector<double>(rgb2.begin(), rgb2.end()));
  if(!mark.empty()) this->set_attribute("mark", mark);
  if(markrgb != std::array<double, 3>{0.0, 0.0, 0.0}) this->set_attribute("markrgb", std::vector<double>(markrgb.begin(), markrgb.end()));
  if(random != 0.0) this->set_attribute("random", random);
  if(gridsize != std::array<int, 2>{0, 0}) this->set_attribute("gridsize", std::vector<int>(gridsize.begin(), gridsize.end()));
...
void Material::set_xml_attrib() const {
  ...
  if(texrepeat != std::array<double, 2>{0.0, 0.0}) {
    this->set_attribute("texrepeat", std::vector<double>(texrepeat.begin(), texrepeat.end()));
  }

  if(texuniform) this->set_attribute("texuniform", texuniform);
  if(emission != 0.0) this->set_attribute("emission", emission);
...
void Hfield::set_xml_attrib() const {
  ...
  if(!name.empty()) this->set_attribute("name", name);
  if(!file.empty()) this->set_attribute("file", file);
  if(!size.empty()) this->set_attribute("size", size);
  if(nrow != 0) this->set_attribute("nrow", nrow);
